...
void duDebugDrawOBBWire(struct duDebugDraw* dd, const float* pos, const float* halfExtends, 
						float rotate, unsigned int col, const float lineWidth)
{
	if (!dd) return;

	const float* hfSize = halfExtends;

	//head,up, right with length
	float head[3] = {dtMathSinf(rotate)*hfSize[2], 0, dtMathCosf(rotate)*hfSize[2]};
	float up[3] = {0.f, 2.f*halfExtends[1], 0.f};
	...
	dd->begin(DU_DRAW_LINES, lineWidth);
	// bottom
	dd->vertex(verts[0], col);
	dd->vertex(verts[1], col);
	dd->vertex(verts[1], col);
	dd->vertex(verts[2], col);
	dd->vertex(verts[2], col);
	dd->vertex(verts[3], col);
	dd->vertex(verts[3], col);
	dd->vertex(verts[0], col);

	// top
	dd->vertex(verts[4], col);
	dd->vertex(verts[5], col);
	dd->vertex(verts[5], col);
	dd->vertex(verts[6], col);
	dd->vertex(verts[6], col);
	dd->vertex(verts[7], col);
	dd->vertex(verts[7], col);
	dd->vertex(verts[4], col);
...
void duDebugDrawOBB(struct duDebugDraw* dd, const float* pos, const float* halfExtends, 
					float rotate, unsigned int col)
{
	if (!dd) return;
