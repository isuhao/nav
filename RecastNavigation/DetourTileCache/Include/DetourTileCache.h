...
{
	enum dtShape{dtCylinder, dtBox};
	dtShape shapeType;
	float pos[3], radius, height;
	float boxHalfExtents[3], fRotate;
...
public:
	...
	dtStatus addObstacle(const float* pos, const float* boxHalfExtents, const float fRotate, dtObstacleRef* result);
