...
void dtGetOBBBounds(const float* pos, const float*halfExtents, const float fRotate, float* bmin, float* bmax )
{
	const float*hfSize = halfExtents;
	//head,up, right with length
	float head[3] = {dtMathSinf(fRotate)*hfSize[2], 0, dtMathCosf(fRotate)*hfSize[2]};
	float up[3] = {0.f, 2.f*hfSize[1], 0.f};
	float right[3]; dtVcross(right, up, head); dtVnormalize(right);
	right[0]*=hfSize[0];right[1]*=hfSize[0];right[2]*=hfSize[0];

	float temp[3] = {0};
	float verts[8][3] = {0};
	dtVadd(temp, head, right);dtVsub(verts[0], pos, temp);//minx, miny, minz,
	dtVsub(temp, head, right);dtVsub(verts[1], pos, temp);//maxx, miny, minz,
	dtVadd(temp, head, right);dtVadd(verts[2], pos, temp);//maxx, miny, maxz,
	dtVsub(temp, head, right);dtVadd(verts[3], pos, temp);//minx, miny, maxz,
	dtVcopy(verts[4],verts[0]);verts[4][1]+=up[1];//minx, maxy, minz,
	dtVcopy(verts[5],verts[1]);verts[5][1]+=up[1];//maxx, maxy, minz,
	dtVcopy(verts[6],verts[2]);verts[6][1]+=up[1];//maxx, maxy, maxz,
	dtVcopy(verts[7],verts[3]);verts[7][1]+=up[1];//minx, maxy, maxz,
	...
	{
		vert = verts[i];
		bmin[0] = dtMin(vert[0], bmin[0]);
		bmin[1] = dtMin(vert[1], bmin[1]);
		bmin[2] = dtMin(vert[2], bmin[2]);

		bmax[0] = dtMax(vert[0], bmax[0]);
		bmax[1] = dtMax(vert[1], bmax[1]);
		bmax[2] = dtMax(vert[2], bmax[2]);
	}
}
 
dtStatus dtMarkBoxArea(dtTileCacheLayer& layer, const float* orig, const float cs, const float ch,
					   const float* pos, const float*halfExtents, const float fRotate, const unsigned char areaId)
 {
 	float bmin[3], bmax[3];
	dtGetOBBBounds(pos, halfExtents, fRotate, bmin, bmax);
	...
	float hfextents[3];// = {halfExtents[0], halfExtents[1], halfExtents[2]};
	hfextents[0] = halfExtents[0] * ics + 0.5f;
	hfextents[1] = halfExtents[1] * ics + 0.5f;
	hfextents[2] = halfExtents[2] * ics + 0.5f;
	...
	//head,up, right with normalized
	float head[3] = {dtMathSinf(fRotate), 0, dtMathCosf(fRotate)};dtVnormalize(head);
	...
	float right[3]; dtVcross(right, up, head); dtVnormalize(right);
 	...
 	for (int z = minz; z <= maxz; ++z)
 	{
 		for (int x = minx; x <= maxx; ++x)
 		{
			const float dis[] = { (float)(x+0.5f)-px, 0.f, (float)(z+0.5f) - pz };

			if( dtMathFabs(dtVdot2D(dis, right)) > hfextents[0] ) continue;
			if( dtMathFabs(dtVdot2D(dis, head)) > hfextents[2] ) continue;
