...
dtStatus dtTileCache::addObstacle( const float* pos, const float* boxHalfExtents, const float fRotate, dtObstacleRef* result )
{
	if (m_nreqs >= MAX_REQUESTS)
		return DT_FAILURE | DT_BUFFER_TOO_SMALL;
	...
	unsigned short salt = ob->salt;
	memset(ob, 0, sizeof(dtTileCacheObstacle));
	ob->shapeType = dtTileCacheObstacle::dtBox;
	ob->salt = salt;
	ob->state = DT_OBSTACLE_PROCESSING;
	dtVcopy(ob->pos, pos);
	dtVcopy(ob->boxHalfExtents, boxHalfExtents);
	ob->fRotate = fRotate;
	...
	for (int i = 0; i < m_params.maxObstacles; ++i)
	{
		...
		if (contains(ob->touched, ob->ntouched, ref))
		{
			if(ob->shapeType==dtTileCacheObstacle::dtBox)
				...
				ob->pos, ob->boxHalfExtents, ob->fRotate, 0);
			else
...
void dtTileCache::getObstacleBounds(const struct dtTileCacheObstacle* ob, float* bmin, float* bmax) const
{
	if (ob->shapeType==dtTileCacheObstacle::dtBox)
	{
		const float*hfSize = ob->boxHalfExtents;
		//head,up, right with length
		float head[3] = {dtMathSinf(ob->fRotate)*hfSize[2], 0, dtMathCosf(ob->fRotate)*hfSize[2]};
		...
		float temp[3] = {0};
		float verts[8][3] = {0};
		dtVadd(temp, head, right);dtVsub(verts[0], ob->pos, temp);//minx, miny, minz,
		dtVsub(temp, head, right);dtVsub(verts[1], ob->pos, temp);//maxx, miny, minz,
		dtVadd(temp, head, right);dtVadd(verts[2], ob->pos, temp);//maxx, miny, maxz,
		dtVsub(temp, head, right);dtVadd(verts[3], ob->pos, temp);//minx, miny, maxz,
