...
InputTerrainGeom::InputTerrainGeom() :
	m_chunkyMesh(0),
	m_mesh(0),
	m_offMeshConCount(0),
	m_volumeCount(0)
...
bool InputTerrainGeom::loadMesh(class rcContext* ctx, const float* verts, unsigned int count)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
	...
	m_mesh = new MeshLoaderStream;
	if (!m_mesh)
	{
		ctx->log(RC_LOG_ERROR, "loadMesh: Out of memory 'm_mesh'.");
		return false;
	}
	if (!m_mesh->load(verts, count))
	{
		ctx->log(RC_LOG_ERROR, "buildTiledNavigation: Could not load");
		return false;
	}
...
#if 0
bool InputTerrainGeom::loadMesh(rcContext* ctx, const char* filepath)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
...
bool InputTerrainGeom::load(rcContext* ctx, const char* filePath)
{
	char* buf = 0;
...
bool InputTerrainGeom::save(const char* filepath)
{
...
bool InputTerrainGeom::raycastMesh(float* src, float* dst, float& tmin)
{
	float dir[3];
	rcVsub(dir, dst, src);
...
void InputTerrainGeom::addOffMeshConnection(const float* spos, const float* epos, const float rad,
...
void InputTerrainGeom::deleteOffMeshConnection(int i)
{
	m_offMeshConCount--;
...
#if 0
void InputTerrainGeom::drawOffMeshConnections(duDebugDraw* dd, bool hilight)
{
...
void InputTerrainGeom::addConvexVolume(const float* verts, const int nverts,
...
void InputTerrainGeom::deleteConvexVolume(int i)
{
	m_volumeCount--;
...
bool InputTerrainGeom::saveNavmeshInput( class rcContext* ctx, const char* filepath )
{
	if(!m_mesh)
		return false;
	return m_mesh->Save(filepath);
}

bool InputTerrainGeom::loadfromNavmeshInput( class rcContext* ctx, const char* filepath )
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
	...
	if (!m_mesh->loadfromnavinput(filepath))
	{
		ctx->log(RC_LOG_ERROR, "buildTiledNavigation: Could not load");
		return false;
	}
...
#if 0
void InputTerrainGeom::drawConvexVolumes(struct duDebugDraw* dd, bool /*hilight*/)
{
	dd->depthMask(false);
